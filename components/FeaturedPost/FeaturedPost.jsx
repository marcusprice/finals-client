...
const FeaturedPost = (props) => {
  ...
  if (
    props.featuredArticle.created_by?.username &&
    props.featuredArticle.created_by?.username !== ''
  ) {
    author = props.featuredArticle.created_by.username;
  } else {
    author =
      props.featuredArticle.created_by.firstname +
      ' ' +
      props.featuredArticle.created_by.lastname;
  }

  const featuredImageURI = props.featuredArticle?.featured_image?.url
    ? process.env.NEXT_PUBLIC_API_ROUTE +
      props.featuredArticle.featured_image.url
    : '/placeholder.png';
  const altText = props.featuredArticle?.featured_image?.alternativeText
    ? props.featuredArticle.featured_image.alternativeText
    : 'gradient placeholder image';
  const title = props.featuredArticle.title;
  const postDate = moment(props.featuredArticle.created_at).format(
    'MMMM Do, YYYY @ h:mma'
  );
  const uriPrefix =
  ...
  return (
    <section className={styles.container}>
      ...
      <h3 className={styles.rainbow + ' hammer'} onClick={() => handleClick()}>
        {title}
      </h3>
      <Link href={'./posts/' + props.featuredArticle.slug}>
        <a>
          <span className={styles.rainbow + ' hammer postInfo'}>
            Posted by {author} on {postDate}
          </span>
        </a>
      </Link>
      <img
        className={styles.featuredImage + ' hammer'}
      ...
      {handleExpanded()}
      {props.commentsOn ? (
        <Comments postID={props.featuredArticle.id} expanded={expanded} />
      ) : (
        ''
      )}
      <span
        className={'readMore hammer ' + styles.rainbow}
        onClick={() => handleClick()}>
